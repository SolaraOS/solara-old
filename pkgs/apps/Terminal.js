import Win from "../../libs/windowSystem.js";
...
export default {
  name: "Terminal",
  description: "Terminal for Solara Developer Preview (forked from Pluto v1.6.2 and Envy 10 2025 Update)",
  ver: "v10.0.1222",
  type: "app",
  privs: 0,
  start: async function (Root) {
    myWindow = new Win.data.win({
      title: "Terminal",
      height: "434px",
      width: "722px"
    });
    ...
    let Terminal = new Html("div")
      .class("no-ui")
      .html(
        /*html*/ `<div><div class="output">Solara [Developer Preview]<br>Copyright (C) Solara Development 2024<br><br></div></div><div class="userInput"><div style="display:flex;" id="inputDiv"><span class="prompt">${Term.prompt}</span><div class="none input" contenteditable="true"></div></div></div>`
      );
    ...
    function setPromptPath() {
      Term.setPrompt(
        `<Span style="color:var(--primary)">${path === "Root" ? "Root/" : path
        }</Span> #&nbsp;`
      );
    }
  ...
  `
          ...
          case "uname":
            console.log(Root);
            let KernelName = "Cherry Tree Core";
            let HostName = window.location.hostname;
            let KernelRelease = Root.systemInfo.version;
            let KernelVersion = Root.systemInfo.codename;
            let Machine = "Browser";
            let OsName = "Envy";
